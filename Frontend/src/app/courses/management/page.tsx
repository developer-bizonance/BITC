...
export default async function ManagementCoursesPage() {
  let dynamicCourses = [];
  try {
    const res = await fetch("https://bitc-backend-theta.vercel.app/api/certifications?category=Management", { cache: "no-store" });
    if (res.ok) {
            ...
            {finalCourses.map((course: any) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1 justify-between">
                  <div>
                    {/* Title */}
                    <h3 className="text-base font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors flex items-center leading-snug mb-2">
                      {course.title}
                    </h3>
                  ...
                  <div className="mt-auto pt-2.5 border-t border-slate-100 space-y-2">
                    ...
                    <Link href={`/courses/${course.id}`} className="block w-full">
                      <Button className="w-full h-10 rounded-full bg-slate-900 hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600 text-white font-bold text-xs transition-all duration-300 shadow-sm cursor-pointer flex items-center justify-center gap-2 group/btn">
